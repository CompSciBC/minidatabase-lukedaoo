...
// The logic of the file remains the same.
// Only the format was changed due to the Editor Formatter
#include <cassert>
#include <cctype>
#include <iostream>
#include <string>
...
int main() {
    ...
    std::vector<Record> seed = {{1000123, "Nguyen", "Anya", "CS", 3.87, false},
    ...
    // --- Test: findById (existing) ---
    {
        ...
        if (rec)
            ts.check(rec->last == "Gonzalez",
                     "findById returns correct record (last name)");
    ...
    // --- Test: rangeById ---
    {
        int cmp = 0;
        auto rows =
            eng.rangeById(1000400, 1001000, cmp);  // expect 1000456, 1000789, 1000811
        ...
        // Visits nodes 1000123,1000456,1000789,1000811,1001022 (5 nodes), ~3 comps/node
        // => 15
    ...
    // --- Test: prefixByLast (case-insensitive) ---
    {
        int cmp = 0;
        auto rows =
            eng.prefixByLast("smi", cmp);  // should match last=="Smith" (two RIDs)
        int cnt = 0;
        for (auto* r : rows)
            if (r->last.rfind("Smith", 0) == 0) ++cnt;
